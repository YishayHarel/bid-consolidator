...
function calcLanded({ fob_price, total_fob, units_per_container, base_duty_pct, addl_duty_pct, etc_amt }) {
  ...
  const units = parseFloat(units_per_container) || 1;
  const etcAmt = parseFloat(etc_amt) ?? 0.10;
  const baseDuty = parseFloat(base_duty_pct) || 0;
  const addlDuty = parseFloat(addl_duty_pct) || 0;

  const vsr_fob = totalFob / 1.12;
  const commission = vsr_fob * commission_pct;
  const duty = fob * (baseDuty + addlDuty);
  const freight_per_unit = units > 0 ? 7500 / units : 0;
  const freight_pct = fob > 0 ? freight_per_unit / fob : 0;
  const landed = fob + commission + duty + freight_per_unit + etcAmt;
...
export default function LandedCostTab() {
    ...
    Promise.all(subs.map(s => api.get(`/submissions/${s.id}/products`).then(r => r.data)))
      .then(results => {
        const flat = results.flat().map(p => ({
          ...
          _base_duty_pct: p.base_duty_pct ?? 0,
          _addl_duty_pct: p.addl_duty_pct ?? 0,
  ...
  const inputStyle = (color) => ({
    width: '72px', padding: '3px 5px', border: '1px solid #cbd5e1', borderRadius: 4,
    fontSize: 12, background: color, textAlign: 'right',
  });
          ...
          <table style={{ borderCollapse: 'collapse', fontSize: 12, minWidth: 1400 }}>
            <thead>
              <tr>
                ...
                <th colSpan={8} style={th('#FFFF00')}>ONLY ENTER HIGHLIGHTED IN VSR</th>
              ...
              {rows.map(row => {
                ...
                return (
                  <tr key={row.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={td('#FFFFCC')}>{fmt(calc.freight_per_unit)}</td>
                    <td style={td('#FFFFCC')}>{fmt(parseFloat(row._etc_amt))}</td>
                    <td style={{ ...td('#FFFFCC'), fontWeight: 700 }}>{fmt(calc.landed)}</td>
                    {/* green */}
                    <td style={td('#CCFFCC')}>
                      <input style={inputStyle('#CCFFCC')} type="number" step="0.01"
                        value={row._sell_price} onChange={e => update(row.id, '_sell_price', e.target.value)} />
                    </td>
                    <td style={td('#CCFFCC')}>{margin !== null ? pct(margin) : '—'}</td>
                    {/* gray */}
                    <td style={td('#f3f4f6')}>
                      <input style={inputStyle('#f3f4f6')} type="number" step="0.01"
                        value={row._retail_price} onChange={e => update(row.id, '_retail_price', e.target.value)} />
                    </td>
                    <td style={td('#f3f4f6')}>{imu !== null ? pct(imu) : '—'}</td>
                    {/* yellow VSR */}
                    <td style={td('#FFFF00')}>
                      <input style={inputStyle('#FFFF00')} type="number" step="0.01"
                        value={row._total_fob} onChange={e => update(row.id, '_total_fob', e.target.value)} />
                    </td>
                    <td style={td('#FFFF00')}>{fmt(calc.vsr_fob)}</td>
                    <td style={td('#FFFF00')}>
                      <input style={inputStyle('#FFFF00')} type="number" step="0.001"
                    ...
                    <td style={td('#FFFF00')}>{pct((parseFloat(row._base_duty_pct)||0) + (parseFloat(row._addl_duty_pct)||0))}</td>
                    <td style={td('#FFFF00')}>
                      <input style={inputStyle('#FFFF00')} type="number"
                        value={row._units_per_container} onChange={e => update(row.id, '_units_per_container', e.target.value)} />
                    </td>
                    <td style={td('#FFFF00')}>{fmt(calc.freight_per_unit)}</td>
                    <td style={td('#FFFF00')}>{pct(calc.freight_pct)}</td>
                    <td style={td('#f8fafc')}>
                      <button
