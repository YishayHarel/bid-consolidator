...
// Zip-bomb guard for untrusted .xlsx uploads (an .xlsx is a zip). Reads only the
// central directory (no decompression) and rejects archives whose declared
// uncompressed size, entry count, or compression ratio is abusive.
const MAX_ENTRIES = 5000;
const MAX_UNCOMPRESSED = 250 * 1024 * 1024; // 250 MB total
const MAX_RATIO = 200;                      // uncompressed : compressed
function assertSafeXlsx(filePath) {
  let zip;
  try { zip = new AdmZip(filePath); } catch { return; } // not a zip (e.g. legacy .xls) — let the parser decide
  const entries = zip.getEntries();
  if (entries.length > MAX_ENTRIES) throw Object.assign(new Error('Spreadsheet has too many parts to process safely.'), { status: 400, expose: true });
  let total = 0, compressed = 0;
  for (const e of entries) { total += e.header.size || 0; compressed += e.header.compressedSize || 0; }
  if (total > MAX_UNCOMPRESSED || (compressed > 0 && total / compressed > MAX_RATIO && total > 20 * 1024 * 1024)) {
    throw Object.assign(new Error('Spreadsheet is too large to process safely.'), { status: 400, expose: true });
  }
}
...
module.exports = { extractImagesFromExcel, extractImagesByRow, assertSafeXlsx };
