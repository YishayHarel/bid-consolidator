...
router.post('/:id/cads', requireAuth, ownProject, cadUploadMw, async (req, res) => {
  ...
  try {
    ...
    const cleanName = `projects/${req.params.id}`; // key root by id, never by (non-unique) name
    ...
    for (const f of req.files) {
      ...
      const key = `${cleanName}/cads/${randomUUID()}${ext}`;
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (let p = 0; p < photos.length; p++) {
        const key = `${cleanName}/excel/images/${randomUUID()}.${photos[p].ext}`;
    ...
    for (const cad of cads) {
      ...
      for (const pageBuf of pages) {
        ...
        for (const prod of products) {
          ...
          const key = `${cleanName}/cads/crops/${randomUUID()}.png`;
...
router.post('/:id/upload', requireAuth, ownProject, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const prefix = `projects/${req.params.id}/quotes/${cleanFactory}`;
...
// Update quote (landed cost data + comparison data). PARTIAL update: only the
// fields present in the body change, so saving notes never wipes the winner flag
// or landed-cost values (and vice-versa). Uses the same has()/CASE pattern as the
// items PATCH. Numeric fields keep a real 0 instead of coercing it to null.
router.patch('/:id/quotes/:qid', requireAuth, ownProject, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE quotes SET
         total_fob           = CASE WHEN $3  THEN $4::numeric  ELSE total_fob           END,
         base_duty_pct       = CASE WHEN $5  THEN $6::numeric  ELSE base_duty_pct       END,
         addl_duty_pct       = CASE WHEN $7  THEN $8::numeric  ELSE addl_duty_pct       END,
         units_per_container = CASE WHEN $9  THEN $10::numeric ELSE units_per_container END,
         sell_price          = CASE WHEN $11 THEN $12::numeric ELSE sell_price          END,
         retail_price        = CASE WHEN $13 THEN $14::numeric ELSE retail_price        END,
         etc_amt             = CASE WHEN $15 THEN $16::numeric ELSE etc_amt             END,
         comparison_notes    = CASE WHEN $17 THEN $18         ELSE comparison_notes    END,
         is_selected_winner  = CASE WHEN $19 THEN $20::boolean ELSE is_selected_winner  END
       WHERE id=$1 AND project_id=$2 RETURNING *`,
      [req.params.qid, req.params.id,
       has('total_fob'), num(b.total_fob),
       has('base_duty_pct'), num(b.base_duty_pct),
       has('addl_duty_pct'), num(b.addl_duty_pct),
       has('units_per_container'), num(b.units_per_container),
       has('sell_price'), num(b.sell_price),
       has('retail_price'), num(b.retail_price),
       has('etc_amt'), num(b.etc_amt),
       has('comparison_notes'), b.comparison_notes ?? null,
       has('is_selected_winner'), !!b.is_selected_winner]
    );
