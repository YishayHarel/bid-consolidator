...
router.post('/send', requireAuth, async (req, res) => {
  const { to, subject, body, project_id } = req.body;
  if (!to || !subject || !body || !project_id) {
    return res.status(400).json({ error: 'to, subject, body and project_id required' });
  }
  if (String(subject).length > 300 || String(body).length > 20000) {
    return res.status(400).json({ error: 'Email is too long' });
  }
  try {
    // Not an open relay: recipients must be contacts of factories invited to a
    // project this user owns. Addresses are checked against the server-side
    // factory records, never trusted from the request alone.
    ...
    const { rows } = await pool.query(
      `SELECT DISTINCT LOWER(e) AS email
         ...
         CROSS JOIN LATERAL unnest(COALESCE(f.emails, ARRAY[]::text[])) AS e
        WHERE pf.project_id = $1`,
      [project_id]
    );
    const allowed = new Set(rows.map(r => r.email));
    const recipients = String(to).split(/[,;\s]+/).map(e => e.trim().toLowerCase()).filter(Boolean);
    if (!recipients.length) return res.status(400).json({ error: 'No recipients' });
    const bad = recipients.filter(e => !allowed.has(e));
    if (bad.length) {
      return res.status(403).json({ error: `Not a contact of a factory on this project: ${bad.join(', ')}` });
    }
    if (!emailTransporter) {
